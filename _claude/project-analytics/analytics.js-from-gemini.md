...
{% macro add_trigger(config, id, selector, action, event_name=none, cat=category.value | default('other') | string) %}
...
{% set config = {
  'vars': {
    'gtag_id': podspec.gaTrackingId | default('G-XXXXXXXX'),
    'config': {
      (podspec.gaTrackingId | default('G-XXXXXXXX')): { 'groups': 'default' }
    }
  },
  'extraUrlParams': {
      'cd1': '${' ~ 'ampdocHost}',
      'debug_mode': 'true',
      '_dbg': '1',
      'page_category': category.value | default('other') | string
    },
  'requests': {
  ...
  'triggers': {
    ...
    'cls': { 
      'on': 'visible', 
      'request': 'CWV_EVENT', 
      'extraUrlParams': { 'cls': '${' ~ 'cumulativeLayoutShift}' } 
    },
    'lcp': { 
      'on': 'visible', 
      'request': 'CWV_EVENT', 
      'extraUrlParams': { 'lcp': '${' ~ 'largestContentfulPaint}' } 
    },
    'fid': { 
      'on': 'visible', 
      'request': 'CWV_EVENT', 
      'extraUrlParams': { 'fid': '${' ~ 'firstInputDelay}' } 
    }
  }
} %}
