...
function buildPixiConfig(environment = 'development') {
  ...
  const endpoints = isProd
    ? siteConfig.apis.pixi.prod
    : siteConfig.apis.pixi.dev;

  return {
    API_ENDPOINT_LINT: endpoints.lint,
    API_ENDPOINT_RECOMMENDATIONS: endpoints.recommendations,
    API_ENDPOINT_CHECK_PAGE_EXPERIENCE:
      siteConfig.apis.pixi.check_page_experience,
...
if (require.main === module) {
  ...
  if (args.includes('--generate-env-configs')) {
    console.log(
      '[config] Generating environment configs from site-vars.yaml...'
    );
