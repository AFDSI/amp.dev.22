...
var CACHE_VERSION = 1;
var CURRENT_CACHES = {
  // These files will be fetched on first service worker initialization and will be present on navigation from AMP to PWA
...
self.addEventListener('install', function (event) {
  ...
  var urlsToPrefetch = ['/de/index.html', '/en/index.html'];

  event.waitUntil(
    caches
      .open(CURRENT_CACHES.prefetch + CACHE_VERSION)
      .then(function (cache) {
...
self.addEventListener('activate', function (event) {
  event.waitUntil(clearUnusedCache());
});
...
var allowedPagesRegExp = new RegExp(
  'automotive-life/|design/|innovation/|performance/|all-models/|bmw-models/|bmw-modelle/',
  'i'
);
...
self.addEventListener('fetch', function (event) {
  ...
  var isEditMode =
    editModeRegExp.test(request.url) || editModeRegExp.test(request.referrer);
  ...
  if (isAAmpPage) {
    ...
    if (isDebug)
      console.log(
        'Handling AMP to PWA fetch event for',
        request.url,
        'now:',
        url
      );
    ...
    if (isDebug)
      console.log('Clean up PWA parameter for', request.url, 'now:', url);
  } else {
    ...
    var promise = fetch(request)
      .then(function (response) {
        ...
        if (response.status == 200) {
          caches
            .open(CURRENT_CACHES.dynamic + CACHE_VERSION)
            .then(function (cache) {
