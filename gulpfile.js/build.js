...
async function buildPlayground() {
  ...
  // Copy Netlify functions for playground
  await gulp
    .src(project.absolute('netlify/functions/**/*'))
    .pipe(gulp.dest(`${project.paths.DIST}/playground/netlify/functions`));
...
/**
 * Generates netlify.toml for go-links site from go-links.yaml
 * Uses environment config for domain parameterization
 */
...
function buildPages(done) {
  return gulp.series(
    ...
    async function packArtifacts() {
      if (!process.env.CI) {
        console.log('packArtifacts: Skipping (not in CI)');
        return;
      }
      ...
      if (fs.existsSync('./dist/inline-examples')) {
        dirs.push('./dist/inline-examples');
      }
...
exports.generateGoLinksConfig = generateGoLinksConfig;
